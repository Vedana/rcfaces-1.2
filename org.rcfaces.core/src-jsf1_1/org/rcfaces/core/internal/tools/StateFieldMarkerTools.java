...
public class StateFieldMarkerTools {
    ...
    private static String getStateMarker(FacesContext facesContext) {
        ...
        if (oldResponseWriter == null) {
            try {
                Class cl = StateFieldMarkerTools.class.getClassLoader()
                        .loadClass("com.sun.faces.RIConstants");

                Field f = cl.getField("SAVESTATE_FIELD_MARKER");

                String marker = (String) f.get(null);

                if (LOG.isDebugEnabled()) {
                    LOG
                            .debug("Get State field marker by introspection method ! Marker="
                                    + marker);
                }
                ...
                LOG.trace("Sun RIConstants introspection failed", th);
            }
