...
/**
 * <p>
 * The pager Component is linked to the <a
 * href="/comps/dataGridComponent.html">dataGrid Component</a>. It shows
 * informations about the result set (ex: number of available pages) and can
 * give direct access to a specific page ("a la Google").
 * </p>
 * <p>
...
public class PagerComponent extends AbstractBasicComponent implements
        IForCapability, IPagerMessageCapability {
    ...
    static {
        CAMELIA_ATTRIBUTES.addAll(Arrays.asList(new String[] {
                "oneResultMessage", "manyResultsMessage", "message",
                "noPagedMessage", "zeroResultMessage", "for" }));
    }
    ...
    /**
     * Returns a string specifying the message to display when there is no row
     * to display.
     * 
     * @return message
     */
    ...
    public String getNoPagedMessage(
            javax.faces.context.FacesContext facesContext) {
        return engine.getStringProperty(Properties.NO_PAGED_MESSAGE,
                facesContext);
    }

    /**
     * Steps a string specifying the message to display when there is no row to
     * display.
     * 
     * @param noPagedMessage
     *            message
     */
