...
public class UIData0 extends UIComponentBase implements NamingContainer,
        UniqueIdVendor {
    ...
    enum PropertyKeys {
         ...
         * <p>
         * Last id vended by
         * {@link UIData#createUniqueId(javax.faces.context.FacesContext, String)}
         * .
         * </p>
         */
        lastId
    ...
    /**
     * <p>
     * Holds the base client ID that will be used to generate per-row client IDs
     * (this will be null if this UIData is nested within another).
     * </p>
     * 
     * <p>
    ...
    /**
     * <p>
     * Length of the cached <code>baseClientId</code> plus one for the
     * {@link UINamingContainer#getSeparatorChar}.
     * </p>
     * 
     * <p>
     * This is not part of the component state.
    ...
    /**
     * <p>
     * Return a flag indicating whether there is <code>rowData</code> available
     * at the current <code>rowIndex</code>. If no <code>wrappedData</code> is
     ...
     * request scope attribute whose key is the <code>var</code> property value.
     * </li>
     * <li>Reset the state information for all descendant components (as
     * described below).
     * </ul>
     ...
     * {@link DataModel}, or a type that can be adapted into a {@link DataModel}. <code>UIData</code> will automatically adapt the following types:
     * </p>
     * <ul>
     * <li>Arrays</li>
    ...
    /**
     * <p>
     * Set the value of the <code>UIData</code>. This value must either be be of
     * type {@link DataModel}, or a type that can be adapted into a
     * {@link DataModel}.
     * </p>
     * 
     * @param value
    ...
    /**
     * <p>
     * If "name" is something other than "value", "var", or "rowIndex", rely on
     * the superclass conversion from <code>ValueBinding</code> to
     ...
     *             {@link #setValueExpression(java.lang.String, javax.el.ValueExpression)}
     *             .
     */
     ...
     * @param binding
     *            The {@link ValueExpression} to set, or <code>null</code> to
     *            remove any currently set {@link ValueExpression}
     * 
     * @throws IllegalArgumentException
     *             if <code>name</code> is one of <code>id</code>,
     *             <code>parent</code>, <code>var</code>, or
     ...
     * value of the <code>rowIndex</code> property, if it is not set to -1. This
     * implies that multiple calls to <code>getClientId()</code> may return
     * different results, but ensures that child components can themselves
     * generate row-specific client identifiers (since {@link UIData} is a
    ...
    public String getClientId(FacesContext context) {
        ...
        if (baseClientId == null && clientIdBuilder == null) {
            if (!isNestedWithinUIData()) {
                ...
                clientIdBuilder.append(UINamingContainer
        ...
        if (rowIndex >= 0) {
            ...
            if (!isNestedWithinUIData()) {
                // Not nested and no row available, so just return our
                // baseClientId
                return (baseClientId);
            } else {
                // nested and no row available, return the result of
                // getClientId().
     ...
     * argument and <b>this</b> as arguments, and return <code>true.</code> If
     * the argument <code>clientId</code> is not equal to
     * <code>this.getClientId()</code>, inspect each of the facet children of
     * this <code>UIData</code> instance and for each one, compare its
     * <code>clientId</code> with the argument <code>clientId</code>. If there
     * is a match, invoke the <code>contextCallback</code>, passing the
     * <code>context</code> argument and <b>this</b> as arguments, and return
     * <code>true</code>. Otherwise, attempt to extract a rowIndex from the
     ...
     * 
     * <p>
     * The implementation of this method must never return <code>true</code> if
     * setting the rowIndex of this instance to be equal to
     ...
     *             {@inheritDoc}
     * @throws FacesException
     *             {@inheritDoc} Also throws <code>FacesException</code> if any
     *             exception is thrown when deriving the rowIndex from the
     *             argument <code>clientId</code>.
     * @since 1.2
     */
    public boolean invokeOnComponent(FacesContext context, String clientId,
                ...
                if (column instanceof UIColumn) {
                    if (column.getFacetCount() > 0) {
                        for (UIComponent facet : column.getFacets().values()) {
                            if (facet.invokeOnComponent(context, clientId,
                                    callback)) {
                                return true;
                            }
                        }
        ...
        try {
            ...
            // myId will be something like form:outerData for a non-nested
            // table,
            ...
            if (clientId.startsWith(myId)) {
                ...
                if (-1 != (preRowIndexSep = clientId.indexOf(sepChar,
                        myId.length()))) {
                    // Check the length
                    if (++preRowIndexSep < clientId.length()) {
                        if (-1 != (postRowIndexSep = clientId.indexOf(sepChar,
                                preRowIndexSep + 1))) {
                            try {
                                newRow = Integer.valueOf(
                                        clientId.substring(preRowIndexSep,
     ...
     * <code>getFacets().keySet().iterator()</code>.</li>
     * <li>Call the <code>processValidators()</code> method of all facets of the
     * {@link UIColumn} children of this {@link UIData}.</li>
     * <li>Iterate over the set of rows that were included when this component
     * was rendered (i.e. those defined by the <code>first</code> and
     * <code>rows</code> properties), performing the following processing for
     ...
     * <code>false</code>, skip further processing.</li>
     * <li>Set the current <code>rowIndex</code> to -1.</li>
     * <li>Call the <code>processUpdates()</code> method of all facets of this
     * {@link UIData}, in the order determined by a call to
     * <code>getFacets().keySet().iterator()</code>.</li>
     ...
     * <li>If <code>isRowAvailable()</code> returns <code>true</code>, iterate
     * over the children components of each {@link UIColumn} child of this
     * {@link UIData} component, calling the <code>processUpdates()</code>
     * method for each such child.</li>
     * </ul>
     * </li>
     * <li>Set the current <code>rowIndex</code> to -1.</li>
     ...
     * 
     * <p>
     * If the {@link UIComponent#isVisitable} method of this instance returns
     * <code>false</code>, take no action and return.
     * </p>
     * 
     * <p>
     * Call {@link UIComponent#pushComponentToEL} and invoke the visit callback
     * on this <code>UIData</code> instance as described in
     * {@link UIComponent#visitTree}. Let the result of the invoctaion be
     * <em>visitResult</em>. If <em>visitResult</em> is
     * {@link VisitResult#COMPLETE}, take no further action and return
     * <code>true</code>. Otherwise, determine if we need to visit our children.
     * The default implementation calls
     * {@link VisitContext#getSubtreeIdsToVisit} passing <code>this</code> as
     * the argument. If the result of that call is non-empty, let
     * <em>doVisitChildren</em> be <code>true</code>. If
     * <em>doVisitChildren</em> is <code>true</code> and <em>visitResult</em> is
     * {@link VisitResult#ACCEPT}, take the following action.
     * <p>
     * 
     * <ul>
     * 
     * <li>
     * <p>
     * If this component has facets, call {@link UIComponent#getFacets} on this
     * instance and invoke the <code>values()</code> method. For each
     * <code>UIComponent</code> in the returned <code>Map</code>, call
     ...
     * <code>UIColumn</code> instance. If such a call returns <code>true</code>,
     * terminate visiting and return <code>true</code> from this method.
     * </p>
     * 
     * <p>
     * If the child <code>UIColumn</code> has facets, call
     ...
     * 
     * <p>
     * For each child component of this <code>UIData</code> that is also an
     * instance of {@link UIColumn},
     * </p>
     * 
     * <p>
     ...
     * <li>
     * <p>
     * While the number of rows processed is less than <em>rowsToProcess</em>,
     ...
     * 
     * <p>
     * If {@link #isRowAvailable} returns <code>false</code>, take no further
     * action and return <code>false</code>.
     * </p>
     * 
     * <p>
     * Call {@link UIComponent#visitTree} on each of the children of this
     ...
     * 
     * <p>
     * Call {@link #popComponentFromEL} and restore the saved row index with a
     * call to {@link #setRowIndex}.
     * </p>
     * 
     * <p>
     ...
     * 
     * @param context
     *            the <code>VisitContext</code> that provides context for
     *            performing the visit.
     * 
     * @param callback
     *            the callback to be invoked for each node encountered in the
     *            visit.
     * 
     * @throws NullPointerException
     ...
     * <p/>
     * <p>
     * If the model has been cached by a previous call to {@link #setDataModel},
     * return it. Otherwise call {@link #getValue}. If the result is null,
     * create an empty {@link ListDataModel} and return it. If the result is an
     * instance of {@link DataModel}, return it. Otherwise, adapt the result as
     * described in {@link #getValue} and return it.
     * </p>
     */
    ...
    /**
     * <p>
     * Set the internal DataModel. This <code>UIData</code> instance must use
     * the given {@link DataModel} as its internal value representation from now
     * until the next call to <code>setDataModel</code>. If the given
     ...
     * the <em>Render Response</em> phase.
     * </p>
     * 
     * @param dataModel
     *            the new <code>DataModel</code> or <code>null</code> to cause
     ...
     * <code>visitTree</code> implementation should visit the rows of UIData or
     * by manipulating the row index before visiting the components themselves.
     * 
    ...
    private void preDecode(FacesContext context) {
        ...
        Map<String, SavedState> saved = (Map<String, SavedState>) getStateHelper()
        ...
        while (true) {
            ...
            if (getChildCount() > 0) {
                for (UIComponent kid : getChildren()) {
                    ...
                    if (renderColumn((UIColumn) kid, phaseId) == false) { // OO
                        continue; // OO
                    } // OO
    ...
    private boolean visitColumnsAndColumnFacets(VisitContext context,
            VisitCallback callback, boolean visitRows) {
        if (visitRows) {
            setRowIndex(-1);
        }
        if (getChildCount() > 0) {
            for (UIComponent column : getChildren()) {
                if (column instanceof UIColumn) {
                    VisitResult result = context.invokeVisitCallback(column,
                            callback); // visit the column directly
                    if (result == VisitResult.COMPLETE) {
                        return true;
                    }
                    if (column.getFacetCount() > 0) {
                        for (UIComponent columnFacet : column.getFacets()
                                .values()) {
