...
public class UIData0 extends UIComponentBase implements NamingContainer {
	...
	public boolean invokeOnComponent(FacesContext context, String clientId,
		...
		try {
			...
			if (clientId.startsWith(myId)) {
				...
				if (-1 != (preRowIndexSep = clientId.indexOf(
						NamingContainer.SEPARATOR_CHAR, myId.length()))) {
					// Check the length
					if (++preRowIndexSep < clientId.length()) {
						if (-1 != (postRowIndexSep = clientId.indexOf(
								NamingContainer.SEPARATOR_CHAR,
								preRowIndexSep + 1))) {
							try {
	...
	public void processDecodes(FacesContext context) {
		...
		if (null == saved || !keepSaved(context)) {
			// noinspection CollectionWithoutInitialCapacity
			saved = new HashMap<String, SavedState>(); // We don't need saved
			// state here
		}
	...
	private boolean keepSaved(FacesContext context) {
	    ...
	    // OO force la sauvegarde !
		return true; //(contextHasErrorMessages(context) || isNestedWithinUIData());
