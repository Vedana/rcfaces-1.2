...
var __members = {
	...
	f_updateVisibility : function(visible) {
		...
		if (!visible) {
			...
			this._oldDisplay = style.display || "$$DEFAULT$$";
			style.display = "none";
			return;
		}
		...
		// Rend visible
		if (this._oldDisplay) {
			// RCFaces JS l'a rendu invisible
			if (this._oldDisplay==="$$DEFAULT$$") {
		...
		if (style.display === "none") {
			// Il est invisible depuis le renderer JAVA
			try {
				style.removeProperty("display");

				// Cela suffit-il ?
				var newDisplay = f_core.GetCurrentStyleProperty(this, "display");
				if (newDisplay !== "none") {
					this._oldDisplay = "$$DEFAULT$$";
					// Il est visible, donc RAS
					return;
				}
				
				// A priori ça ne suffit pas à le rendre visible
			} catch (x) {
				// Merci IE ...
			}
		}

		// On va chercher le display par defaut pour ce composant
