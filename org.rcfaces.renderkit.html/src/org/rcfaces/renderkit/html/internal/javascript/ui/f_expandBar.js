 ...
 * class f_expandBar
 * 
 * @class public f_expandBar extends f_component, fa_disabled, fa_readOnly,
 *        fa_collapsed, fa_groupName, fa_overStyleClass, fa_asyncRender
...
var __members = {
	f_expandBar : function() {
		...
		var userExpandable = f_core.GetBooleanAttributeNS(this,
				"userExpandable", true);
		...
		if (head) {
			...
			var text = doc.getElementById(this.id
					+ f_expandBar._LABEL_ID_SUFFIX);
			if (text) {
				this._text = text;
				text._link = this;
		...
		if (body) {
			...
			this._content = doc.getElementById(this.id
					+ f_expandBar._CONTENT_ID_SUFFIX);
		}
	...
	f_getButton : function() {
		...
		button = this.ownerDocument.getElementById(this.id
				+ f_expandBar._BUTTON_ID_SUFFIX);
		if (button) {
			button.f_link = this;
		}
	...
	f_setFocus : function() {
		...
		if (cmp.visible) { // IE
			f_core.Debug(f_expandBar, "f_setFocus: focus component '" + cmp
					+ "' for expandBar '" + this.id + "'.");
			cmp.focus();
		}
	},
	/**
	...
	fa_updateCollapsed : function(set) {
		...
		if (button) {
			...
			if (set) {
				...
				alt = f_resourceBundle.Get(f_expandBar)
						.f_get("COLLAPSE_BUTTON");
			}
		...
		if (collapsedText && this._text) {
			...
			f_core.Debug(f_expandBar, "fa_updateCollapsed: Change text to '"
					+ collapsedText + "'.");
	...
	f_getEffect : function() {
		...
		if (effect) {
			effect = f_core
					.CreateEffectByName(effect, content,
							function(value) {
	...
	/**
	 * Returns the group name of this expandBar, or <code>null</code> if it is
	 * not defined !
	 * 
	 * @method public
	...
	f_setText : function(text) {
		f_core.Assert(typeof (text) == "string",
				"f_expandBar.f_setText: Invalid text parameter ! ('" + text
						+ "')");
	...
	f_setCollapsedText : function(text) {
		f_core.Assert(typeof (text) == "string",
				"f_expandBar.f_setCollapsedText: Invalid text parameter ! ('"
						+ text + "')");
	...
	f_updateHeight : function(height) {
		...
		var border = f_core
				.ComputeContentBoxBorderLength(this, "top", "bottom");
		height -= border;
		...
		var body = this._body;
		if (body) {
			var margin = f_core.ComputeMarginsLength(body, "top", "bottom");
			height -= margin;
			...
			if (content && content != body) {
				var borderBody = f_core.ComputeContentBoxBorderLength(body,
						"top", "bottom");
				height -= borderBody;

				margin = f_core.ComputeMarginsLength(content, "top", "bottom");
				height -= margin;
...
new f_class("f_expandBar", {
	...
	aspects : [fa_disabled, fa_readOnly, fa_collapsed, fa_groupName,
			fa_overStyleClass, fa_asyncRender],
	statics : __statics,
	members : __members
});
