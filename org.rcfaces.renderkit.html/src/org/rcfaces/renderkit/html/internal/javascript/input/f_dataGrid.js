...
//		f_core.Assert(!this._loading, "Already loading ....");
	...
	f_initializeTableLayout0: function() {
		...
		if (this._sb) {
			var gridWidth=this.style.width;
			var gridHeight=this.style.height;
		
			this._scrollTitle=f_core.CreateElement(this, "div", {className: "f_grid_dataTitle_scroll"});
			if (gridWidth) {
				this._scrollTitle.style.width=gridWidth;
			}

			title=f_core.CreateElement(this._scrollTitle, "table", {className: "f_grid_fttitle", cellPadding:0, cellSpacing:0 });
			this._title=title;
			
			f_core.CreateElement(title, "thead");
			f_core.CreateElement(title, "tbody");
	
			tableBody=f_core.CreateElement(this, "div", {className: "f_grid_dataBody_scroll"});
			if (gridWidth) {
				tableBody.style.width=gridWidth;
			}			
			if (gridHeight) {
			...
			this._scrollBody=tableBody;
			tableBody._dataGrid=this;
		
			tableBody.onscroll=f_grid._OnScroll;

			if (f_core.IsGecko()) {
				tableBody.addEventListener("DOMMouseScroll", f_grid._Link_onmousewheel, false);
			}
		}

		table=f_core.CreateElement(tableBody, "table", {className: "f_grid_table", cellPadding:0, cellSpacing:0 });
		if (!title) {
			table.width="100%";
			f_core.CreateElement(table, "thead");
		}
		...
		this._tbody=table.ownerDocument.createElement("tbody");
	...
	*/
	...
	f_updateColumnsLayout0: function(columns) {
		...
		var headGrid=titleGrid;
		if (!headGrid) {
			headGrid=tableGrid;
		}
		var thead=headGrid.firstChild;
		
		var headRow=f_core.CreateElement(thead, "tr", {className: "f_grid_title"});
		...
		for(var i=0;i<columns.length;) {
			...
			if (column._visibility) {
				var col=headRow.ownerDocument.createElement("col");
				headGrid.insertBefore(col, thead);
				column._col=col;					
				if (colWidth) {
				...
				if (titleGrid) {
					// On ajoute à la table aussi ...
					
					var col2=headRow.ownerDocument.createElement("col");
					tableGrid.insertBefore(col2, this._tbody);
					if (colWidth) {
						col2.style.width=colWidth;
						
						column._col2=col2;
					}
				}

				var head=f_core.CreateElement(headRow, "th", {className: "f_grid_tcell"});
				column._head=head;
				head._column=column;
				if (colWidth) {
					head.style.width=colWidth;
				}
				...
				column._box=f_core.CreateElement(head, "div", {className: "f_grid_stext"});
				column._label=f_core.CreateElement(column._box, "div", {className: "f_grid_ttext"});

				var image=f_core.GetFirstElementByTagName(column._label, "img");
				if (column._titleImageURL) {
					column._image=f_core.CreateElement(column._label, "img", {width:15, height: 15, src: column._titleImageURL});
				}

				if (column._text) {
					f_core.SetTextNode(column._label, column._text);
				}

				if (column._resizable) {
					var cursor=f_core.CreateElement(column._box, "div", {className:"f_grid_colCursor", title: headCursorTitle});
		...
		if (this._sb) {
			...
			var th=f_core.CreateElement(headRow, "th", {className: "f_grid_tcell"});
			th.innerHTML="&nbsp;";
			th.style.width=scrollBarWidth+"px";
			
			this._fakeColWidth=scrollBarWidth;
		}
		
		if (totalWidth>=0 && this._sb) {
			tableGrid.style.width=totalWidth+"px";
			if (titleGrid) {
				titleGrid.style.width=totalWidth+"px";
			}
		}
	},
	*/
