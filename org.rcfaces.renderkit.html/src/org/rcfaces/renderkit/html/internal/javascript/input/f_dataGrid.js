 ...
 *        fa_droppable, fa_draggable, fa_autoOpen
...
var __statics = {
    ...
    _CheckMouseButtons: function(evt) {
        var row = this._row;
        var dataGrid = row._dataGrid;
        ...
        //console.log("_CheckMouseButtons: perform event " + evt, row._check);
        ...
        if (dataGrid._checkCardinality !== fa_cardinality.ONE_CARDINALITY) {
            return;
        }
    ...
    _CheckSelect: function (evt) {
        ...
        //console.log('_CheckSelect', 'rowId=', row.id, 'checked=', checked, 'row._checked=', row._checked);
    ...
    _AdditionalInformationSelect: function (evt) {
        ...
        if (dataGrid.f_hasAdditionalElement(row)) {
            ...
            dataGrid.fa_performElementAdditionalInformation(row, true, evt,
                show);
        }
...
var __members = {
    ...
    /*
     * f_serialize : function() { return this.f_super(arguments); },
     */
    ...
    fa_updateElementStyle: function (row, updateCells) {
        ...
        //console.log('fa_updateElementStyle', 'rowId=', row.id, 'row._checked=', row._checked, 'checkbox.checked=', row._checkbox.checked, 'updateCells=', updateCells);
        ...
        if (updateCells !== false) {
            ...
            if (input && checked !== input.checked) {
                ...
                var bundleKey = "CHECK_TITLE";

                if (checked) {
                    if (this._checkCardinality == fa_cardinality.ONE_CARDINALITY) {
                        bundleKey = "CHECKED_TITLE";
                    } else {
                        bundleKey = "UNCHECK_TITLE";
                    }
                }

                input.title = rb.f_formatParams(bundleKey, {
    ...
    f_addRow2: function (value, properties, columnValue1) {
        ...
        if (shadowRows && shadowRows.length) {
            ...
            f_core.Assert(row.tagName.toLowerCase() == "tr",
                "f_dataGrid.f_addRow2: Invalid row ! " + row);
        ...
        if (this.f_isSelectable() || this.f_isCheckable() || this._additionalInformations) {
            ...
            //row.onclick = f_grid.RowMouseDown;
        ...
        for (var i = 0; i < columns.length; i++) {
            ...
            if (col._visibility === null) {
                ...
                if (col._visibility) {
                    ...
                    if (colStyleClasses) {
                        var csc = colStyleClasses[rowIdx
                        % colStyleClasses.length];
                        if (csc) {
                            var cs = csc.split(" ");
                    ...
                    td.setAttribute("aria-labelledby", this.id + "::ch"
                        + col._index + " cellIdx" + (f_dataGrid._CellIdx));
                    td.valign = "top";
                    if (!cellWrap) {
                        td.noWrap = true;
                    }
                    ...
                    if (!countTd) {
                        if ((this._additionalInformations || this
                                .f_isCheckable())
                            && f_core.IsInternetExplorer()) {
                        ...
                        if (this._additionalInformations) {
                            ...
                            if (this._additionnalOpenImageURL === undefined) {
                                this._additionnalOpenImageURL = f_core
                                    .GetAttributeNS(this,
                                        "addOpenImageURL", undefined);
                                if (this._additionnalOpenImageURL) {
                                    this._additionnalCloseImageURL = f_core
                                        .GetAttributeNS(this,
                                            "addCloseImageURL");
                        ...
                        if (this.f_isCheckable()) {
                            ...
                            input.onchange = f_dataGrid._CheckSelect;
                            input.onmousedown = f_dataGrid._CheckMouseDown;
                            input.onmouseup = f_dataGrid._CheckMouseDown;
                            ...
                            if (this._focusOnInput) {
                                ...
                                input.tabIndex = -1; // -1 car sinon pas de
                                // sortie du focus du
                                // grid
                    ...
                    if (col._cellImage || col._defaultCellImageURL) {
                        ...
                        // L'image par cellule est spécifié par une méthode
                        // evaluée plus tard ...

                        if (f_grid.USE_BACKGROUND_IMAGE) {
                            cellImage.src = this._blankImageURL;
                            if (imageURL) {
                                cellImage.style.backgroundImage = "url("
                                    + imageURL + ")";
                            }
                    ...
                    f_core.AppendChild(labelComponent, doc
                        .createTextNode(cellText));

                    if (col._cellClickable) {
                        fa_audioDescription.AppendAudioDescription(
                            labelComponent, clickableCellMessage);
                    }
                    if (td._toolTipId) {
                        fa_audioDescription.AppendAudioDescription(
                            labelComponent, tooltipCellMessage);
    ...
    f_clear: function (rowValue1) {
        f_core
            .Assert(
                this._rows == 0,
                "f_dataGrid.f_clear: All rows of the DataGrid must be loaded (attribute rows=0)");
     ...
     * @method public
     * @param any
     *            rowValue Row value, a row object, or the index of row into the
     *            table.
    ...
    f_getRowValues: function (rowValue, onlyVisible) {
        f_core.Assert(rowValue !== undefined && rowValue !== null,
            "f_dataGrid.f_getRowValues: Invalid rowValue parameter ! ("
            + rowValue + ")");
        f_core.Assert(onlyVisible === undefined
            || typeof (onlyVisible) == "boolean",
            "f_dataGrid.f_getRowValues: Invalid onlyVisible parameter ! ("
            + onlyVisible + ")");
        var row;
    ...
    f_getRowValuesSet: function (rowValue, onlyVisible) {
        ...
        f_core.Assert(onlyVisible === undefined
            || typeof (onlyVisible) == "boolean",
            "f_dataGrid.f_getRowValuesSet: Invalid onlyVisible parameter ! ("
            + onlyVisible + ")");
     ...
     * @method protected
     * @param any
     *            rowValue Row value, row object or the index of row the into
     *            table.
     * @param Number
     ...
     * @method protected
     */
    f_callServer: function (firstIndex, length, cursorIndex, selection,
                            partialWaiting, fullUpdate) {
        var indexDb = this._indexDb;
        if (!indexDb || this._sortIndexes || this._additionalInformations
            || this._selectedCriteria || fullUpdate) {
            return this._ajaxCallServer(firstIndex, length, cursorIndex,
                selection, partialWaiting, fullUpdate);
        }
        ...
        indexDb.f_asyncSearch(null, firstIndex, rows, function (state) {
            if (!state) {
                return self._ajaxCallServer(firstIndex, length, cursorIndex,
                    selection, partialWaiting, fullUpdate);
            }
    ...
    _prepareNewRows: function (params, cursorIndex, selection, partialWaiting) {
        ...
        if (!partialWaiting) {
            ...
            if (tbody) {
                ...
                if (this._additionalInformations) { // Des
                    // AdditionalInformations à
                    // effacer ?
                    ...
                    f_core.Debug(f_dataGrid, "f_callServer: garbage "
                ...
                if (tbody.parentNode) {
                    ...
                    // if (tbody.parentNode.nodeType !=
                    // f_core.DOCUMENT_FRAGMENT) {
                    f_core.Assert(tbody.parentNode == this._table,
                        "f_dataGrid.f_callServer: Not same parent ? ("
                        + tbody.parentNode + ")");
                ...
                // Il faut dettacher les composants afin de retrouver ceux qui
                // doivent être garbagés
     ...
     *            cursorIndex
     */
    _ajaxCallServer: function (firstIndex, length, cursorIndex, selection,
                               partialWaiting, fullUpdate) {
        ...
        if (fullUpdate || this.f_isRefreshFullUpdateState()
        ...
        f_core.Debug(f_dataGrid, "f_callServer: Call server  firstIndex="
            + firstIndex + " cursorIndex=" + cursorIndex + " selection="
            + selection);
        ...
        var request = new f_httpRequest(this,
            f_httpRequest.JAVASCRIPT_MIME_TYPE);
        var dataGrid = this;
        request
            .f_setListener({
                /**
                ...
                onInit: function (request) {
                    if (!waitingObject) {
                        waitingObject = f_waiting.Create(dataGrid, null,
                            false);
                        dataGrid._waiting = waitingObject;
                    }

                    if (waitingObject) {
                        waitingObject.f_setText(f_waiting
                            .GetLoadingMessage());
                ...
                onError: function (request, status, text) {
                    f_core.Info(f_dataGrid,
                        "f_callServer.onError: Bad status: " + status);

                    var continueProcess;

                    try {
                        continueProcess = dataGrid.f_performErrorEvent(
                            request, f_error.HTTP_ERROR, text);
                ...
                onProgress: function (request, content, length, contentType) {
                    if (waitingObject
                        && f_classLoader
                            .IsObjectInitialized(waitingObject)) {
                        waitingObject.f_setText(f_waiting
                            .GetReceivingMessage());
                    }
                },
                ...
                onLoad: function (request, content, contentType) {
                    ...
                    try {
                        if (request.f_getStatus() != f_httpRequest.OK_STATUS) {
                            dataGrid.f_performErrorEvent(request,
                        ...
                        if (responseContentType
                            ...
                            dataGrid.f_performErrorEvent(request, code,
                                content);
                            return;
                        }

                        if (responseContentType
                            .indexOf(f_httpRequest.JAVASCRIPT_MIME_TYPE) < 0) {
                            dataGrid.f_performErrorEvent(request,
    ...
    f_startNewPage: function (rowIndex) {
        ...
        if (false) {
            ...
            if (tbody && tbody.parentNode) {
                ...
                f_core.Assert(tbody.parentNode == this._table,
                    "StartNewPage: Not same parent ? (" + tbody.parentNode
                    + "/" + this._table + ")");
                this._table.removeChild(tbody);
            ...
            if (this.f_isSelectable()) {
                ...
                if (oldCurrentSelection) {
                    ...
                    if (!this._selectionFullState) {
                        ...
                        this.fa_fireSelectionChangedEvent(null, {
                            value: f_event.REFRESH_DETAIL,
                            refresh: true
                        });
                    }
                }
    ...
    f_updateNewPage: function () {
        ...
        f_core.Debug(f_dataGrid, "f_updateNewPage: Update new page _rowCount='"
            + this._rowCount + "' _maxRows=" + this._maxRows + "' _rows='"
            + this._rows + "'.");
        ...
        if (tbody && !this._partialWaiting) {
            f_core
                .Assert(tbody.parentNode != this._table,
                    "f_dataGrid.f_updateNewPage: Tbody has not been detached !");
        ...
        if (this._inputTabIndex) {
            f_core.Debug(f_dataGrid, "f_updateNewPage: give focus to '"
                + this._inputTabIndex + "'.");
    ...
    f_setCells2: function (row, configs) {
        var tds = row._cells; // getElementsByTagName("td"); // Parfois ca
        // peut être du TH !
            ...
            if (toolTipText) {
                ...
                if (toolTipId) {
                    ...
                    if (td._label && !td._toolTipId) {
                        if (!tooltipMessage) {
                            tooltipMessage = f_resourceBundle.Get(f_dataGrid)
                                .f_get("TOOLTIP_CELL");
                        }
     ...
     * @param Boolean
     *            show Show the checked row.
     * @param hidden
     *            optional Event jsEvent
     * @return Boolean Returns <code>true</code> if check has successed.
     */
    f_checkRow: function (rowValue, show, jsEvent) {
        var row = this.f_getRowByValue(rowValue, true);

        //console.log('f_checkRow', 'rowId=', row.id, 'rowValue=', rowValue, 'show=', show, 'isChecked=', this.fa_isElementChecked(row), 'row._checked=', row._checked, 'checkBox.checked=', row._checkbox.checked);
    ...
    fa_isElementChecked: function (row) {
        f_core.Assert(row && row.tagName.toLowerCase() == "tr",
            "f_dataGrid.fa_isElementChecked: Invalid element parameter ! ("
            + row + ")");
    ...
    fa_setElementChecked: function (row, checked) {
        f_core.Assert(row && row.tagName.toLowerCase() == "tr",
            "f_dataGrid.fa_setElementChecked: Invalid element parameter ! ("
            + row + ")");

        //console.log('fa_setElementChecked', 'rowId=', row.id, 'row._checked=', row._checked, 'checked=', checked);
    ...
    f_sortClientSide: function (methods, ascendings, tdIndexes) {
        ...
        function internalSort(obj1, obj2) {
            for (var i = 0; i < methods.length; i++) {
                ...
                var ret = methods[i].call(self, tc1._text, tc2._text, tc1, tc2,
                    tc1._index, tc2._index);
                if (!ret) {
                    continue;
                }
    ...
    f_searchRowNode: function (code, evt, selection) {
        ...
        if (this._lastKeyDate !== undefined) {
            ...
            f_core.Debug(f_dataGrid, "_searchRowNode: Delay key down " + dt
                + "ms");
     ...
     * @method private
     * @param String
     *            key Complete text to search
     * @param Boolean
     *            next Skip the current row
     * @param Event
     *            evt
     * @param Boolean
     *            selection
    ...
    f_getElementLabel: function (rowValue) {
        ...
        if (labelColumnId === undefined) {
            labelColumnId = f_core.GetAttributeNS(this, "rowLabelColumnId",
                null);
    ...
    fa_evaluateCriteria: function (selectedCriteria, callBack, waitingElement) {
        ...
        this
            .f_appendCommand(function (dataGrid) {

                var params = new Object();
                params.gridId = this._serviceGridId;
                params.tokenId = this._countToken;

                params.selectedCriteria = this
                    ._computeSelectedCriteria(selectedCriteria);
                ...
                request
                    .f_setListener({
                        ...
                        onInit: function (request) {
                            if (waitingElement) {
                                f_core.SetTextNode(waitingElement,
                                    f_waiting.GetLoadingMessage());
                            }
                        },
                        /**
                         * @method public
                        ...
                        onError: function (request, status, text) {
                            f_core.Info(f_dataGrid,
                                "f_callServer.onError: Bad status: "
                                + status);

                            try {
                                continueProcess = dataGrid
                                    .f_performErrorEvent(request,
                                        f_error.HTTP_ERROR,
                                        text);
                         ...
                         * @method public
                         */
                        onProgress: function (request, content, length,
                                              contentType) {
                            if (waitingElement) {
                                f_core
                                    .SetTextNode(
                        ...
                        onLoad: function (request, content, contentType) {
                            ...
                            var event = new f_event(dataGrid,
                                f_event.LOAD);
                            try {
                                dataGrid.f_fireEvent(event);
                ...
                this._loading = true;
                request.f_setRequestHeader("X-Camelia",
                    this._gridUpdadeCriteriaServiceId);
                request.f_doFormRequest(params);
            });
     ...
     * @method protected
     * @param Array
     *            criteriaSelected Sous la forme [{ id: "idColonne", values: [
     *            val1, val2 ] }, {...} ]
     * @param Boolean
    ...
    fa_setSelectedCriteria: function (selectedCriteria, refresh) {
        f_core.Assert(selectedCriteria instanceof Array,
            "f_dataGrid.fa_setSelectedCriteria: Invalid selectedCriteria parameter ! ("
            + selectedCriteria + ")");
    ...
    fa_getColumnCriteriaCardinality: function (columnId) {
        f_core.Assert(typeof (columnId) == "string"
            || typeof (columnId) == "object",
            "f_dataGrid.fa_getColumnCriteriaCardinality: Invalid columnId parameter ! ("
            + columnId + ")");
     ...
     * @return void
     */
    _processSelectedCriteriaResult: function (tokenId, resultCount,
                                              availableCriteria) {
     ...
     * @method private
     * @param columnId
     *            Identifier of Column or column object
     ...
     * @method public
     * @param String
     *            columnId Identifier of Column or column object
     */
    fa_getCriteriaLabelByColumn: function (columnId) {
        f_core.Assert(typeof (columnId) == "string"
            || typeof (columnId) == "object",
            "f_dataGrid.fa_getCriteriaLabelByColumn: Invalid columnId parameter ! ("
            + columnId + ")");
    ...
    fa_showElement: function (row, giveFocus) {
        ...
        if (row && row._input) {
            ...
            /*
             * if (this._scrollBody && this._checkable &&
             * this._scrollBody.scrollLeft) { var
             * sl=this._scrollBody.scrollLeft; sl-=input.offsetLeft; if (sl>0) {
             * deltaFocus={ x: sl }; } }
             */
