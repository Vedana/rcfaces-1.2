...
/**
 * <p>
 * <strong>f_shellManager</strong> represents shell manager.
 * 
...
var __statics = {
	...
	/**
	 * <p>
	 * For IE 6 only : Hide selects that get over the Div
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * For IE 6 only : Show selects that get over the Div
	 * </p>
	 * 
	 * @method protected static
	 * @return void
	...
	_OnFocus : function(evt) {
		...
		f_core.Debug(f_shellManager, "_OnFocus: entering on " + this.tagName
				+ "#" + this.id + "." + this.className + " event=" + evt);
		...
		if (shellDecorator.f_isIntoShell(target)) {
			f_core.Info(f_shellManager, "_OnFocus: Shell decorator="
					+ shellDecorator + " into shell " + target);
			return true;
		}
		f_core.Info(f_shellManager, "_OnFocus: Shell decorator="
				+ shellDecorator + " set focus " + target);
	...
	GetScreenSize : function(doc) {
		...
		var size = {
			width : (viewSize.width > docSize.width)
					? viewSize.width
					: docSize.width,
			height : (viewSize.height > docSize.height)
					? viewSize.height
					: docSize.height
		};

		// document.title="viewSize:"+viewSize.width+","+viewSize.height+"
		// docSize="+docSize.width+","+docSize.height+"
	...
	GetShell : function(component) {
		f_core.Assert(component,
				"f_shellManager.GetShell: Invalid component parameter '"
						+ component + "'.");
		...
		try {
			for (; component;) {
				...
				if (component._rcfacesShellDecoratorIdentifier) {
					...
					var shell = win.f_shellDecorator
							.GetShellFromIdentifier(component._rcfacesShellIdentifier);
					return shell;
				}
			...
			f_core.Error(f_shellManager, "Can not find shell from root='"
					+ root.id + "' and component='" + component.id + "'.", ex);
		}
	 ...
	 * @method public static
	 * @param Object
	 *            object A f_event object or a component (rcfaces or html)
	 * @param any
	 *            returnValue
	 * @return Boolean Returns <code>true</code> if the shell is found.
	 */
	CloseShell : function(object, returnValue) {
		f_core.Debug(f_shellManager, "CloseShell: Request close shell '"
		...
		if (!object
				|| (object.nodeType != f_core.ELEMENT_NODE && object.nodeType != f_core.DOCUMENT_NODE)) {
			return false;
		}
	...
	Finalizer : function() {
		...
		if (shellManager) {
			...
			if (shellManager._modalStyleInstalled) {
				...
				if (f_core.IsInternetExplorer(7)
						|| f_core.IsInternetExplorer(8)) {
					capture = undefined;
				}

				f_core.RemoveEventListener(document, "focus",
						f_shellManager._OnFocus, capture);
			}
		}
...
var __members = {
	...
	f_pushShell : function(shell) {
		...
		if (shell.f_getStyle()
				& (f_shell.PRIMARY_MODAL_STYLE | f_shell.APPLICATION_MODAL_STYLE)) {
			this.f_installModalStyle(shell);
		}
	...
	_hideScreen : function() {
		...
		div.className = "f_shellManager_background f_shellManager_background_"
				+ backgroundMode;
	...
	f_installModalStyle : function(shell) {
		...
		f_core
				.Debug(f_shellManager,
						"f_installModalStyle: Install modal hooks");
		...
		var shellBody = shell.f_getBaseComponent();

		for (var i = 0; i < bodyChildren.length; i++) {
			var child = bodyChildren[i];

			if (child.id === shellBody.id) {
				continue;
			}
	...
	f_uninstallModalStyle : function() {
		f_core.Debug(f_shellManager,
				"f_uninstallModalStyle: Uninstall modal hooks");
		...
		for (var i = 0; i < bodyChildren.length; i++) {
			...
			if (child.hasAttribute("f_aria-hidden")) {
				...
				child.removeAttribute("f_aria-hidden");
				continue;
			}
	 ...
	 * @param f_shell
	 *            shell
	 * @param f_shellDecorator
	 *            shellDecorator
	 * @return void
	 */
	f_setShellDecorator : function(shell, shellDecorator) {
		f_core.Debug(f_shellManager,
				"f_getShellDecorator: create new shell decorator: "
						+ shellDecorator);
	...
	f_openShell : function(shell) {
		if (!f_shellManager._documentComplete) {
			f_core
					.Debug(f_shellManager,
							"f_openShell: document is not complete, push shell into pipe !");
		...
		var self = this;
		this.f_getShellDecorator(shell).f_createDecoration(
				function(shellDecorator, shell) {
					f_core.Debug(f_shellManager,
							"f_openShell: creation started ...");
	...
	f_closeShell : function(shell, showNextShell) {
		f_core.Debug(f_shellManager, "f_closeShell: Requested close shell '"
				+ shell + "' showNextShell='" + showNextShell + "'");
		if (shell.f_getStatus() == f_shell.CREATED_STATUS) {
			shell.f_setStatus(f_shell.DESTROYING_STATUS); // Directement ...
		}

		if (shell.f_getStatus() == f_shell.OPENING_STATUS
				|| shell.f_getStatus() == f_shell.OPENED_STATUS) {
		...
		if (shell.f_getStatus() == f_shell.CLOSING_STATUS) {
			try {
				...
				f_core.Error(f_shellManager,
						"f_closeShell: f_hideShell throws exception self="
								+ self, x);
			}

			try {
				shell.f_preDestruction(); // C'est le preDestruction qui
				// positionne le status
				// ABOUT_TO_CLOSE ...

			} catch (x) {
				f_core.Error(f_shellManager,
						"f_closeShell: f_preDestruction throws exception self="
								+ self, x);
			}
		}
		...
		if (shell.f_getStatus() == f_shell.ABOUT_TO_CLOSE_STATUS) {
			try {
				...
				f_core.Error(f_shellManager,
						"f_closeShell: f_destroyDecoration throws exception self="
								+ self, x);
			}
		}
		...
		if (shell.f_getStatus() == f_shell.CLOSED_STATUS) {
			try {
				...
				f_core.Error(f_shellManager,
						"f_closeShell: f_popShell throws exception self="
								+ self, x);
			}
		...
		if (shell.f_getStatus() == f_shell.DESTROYING_STATUS) {
			try {
				...
				f_core.Error(f_shellManager,
						"f_closeShell: postDestruction throws exception self="
								+ self, x);
			}
		}
	...
	f_setShellBounds : function(shell, x, y, width, height) {
		this.f_getShellDecorator(shell).f_setShellBounds(shell, x, y, width,
				height);
	},
	/**
	 * @method public
