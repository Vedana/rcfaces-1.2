...
var __statics = {
	...
	VK_F12 : 0x7B,
	/**
	 * @field public static final Number VK_F13 : 0x7C,
	 */
	/**
	 * @field public static final Number VK_F14 : 0x7D,
	 */
	/**
	 * @field public static final Number VK_F15 : 0x7E,
	 */
	/**
	 * @field public static final Number VK_F16 : 0x7F,
	 */
	/**
	 * @field public static final Number VK_F17 : 0x80,
	 */
	/**
	 * @field public static final Number VK_F18 : 0x81,
	 */
	/**
	 * @field public static final Number VK_F19 : 0x82,
	 */
	/**
	 * @field public static final Number VK_F20 : 0x83,
	 */
	/**
	 * @field public static final Number VK_F21 : 0x84,
	 */
	/**
	 * @field public static final Number VK_F22 : 0x85,
	 */
	/**
	 * @field public static final Number VK_F23 : 0x86,
	 */
	/**
	 * @field public static final Number VK_F24 : 0x87,
	 */
	...
	IsPrintable : function(code) {
		if (f_key.IsLetterOrDigit(code) || (code == f_key.VK_SPACE)
	...
	ExitScope : function(scopeName) {
		f_core.Assert(f_key._CurrentScopes,
				"f_key.ExitScope: No scopes defined.");
		f_core.Assert(f_key._CurrentScopes.length,
				"f_key.ExitScope: Empty scopes !");

		var scope = f_key._CurrentScopes.pop();
		f_core.Assert(scope._name == scopeName,
				"f_key.ExitScope: Bad scope ! [removed='" + scope._name
						+ "', asked='" + scopeName + "']");
	...
	SetAltKeyMode : function(scopeName, needAltKey) {
		...
		f_core.Assert(scope, "f_key.SetAltKeyMode: Scope '" + scopeName
				+ "' not found.");
	...
	AddKeyHandler : function(scopeName, keyName, component, method, parameter) {
		...
		f_core.Assert(!scope[keyName], "f_key.AddKeyHandler: The scope '"
				+ scopeName + "' already contains a definition for the key '"
				+ keyName + "'.");
		...
		f_core.Debug(f_key, "AddKeyHandler: Add key handler scope='"
				+ scopeName + "' keyName='" + keyName + "'"
	 ...
	 * @return void
	 */
	AddAccelerator : function(character, virtualKeys, keyFlags, component,
			method, ignoreEditableComponent) {
		var scope = f_key._GetScopeByName();
		...
		f_core.Debug(f_key, "AddAccelerator: Add accelerator character='"
				+ character + "' virtualKeys='" + virtualKeys + "' keyFlags='"
				+ keyFlags + "'.");
	...
	RemoveKeyHandler : function(scopeName, keyName) {
		f_core.Assert(f_key._Scopes,
				"f_key.RemoveKeyHandler: No scopes defined.");

		var scope = f_key._Scopes[scopeName];
		f_core.Assert(scope, "f_key.RemoveKeyHandler: Scope '" + scopeName
				+ "' not found.");
	...
	RemoveScope : function(scopeName) {
		...
		var scopes = f_key._Scopes;
		var scope = scopes[scopeName];
		f_core.Assert(scope, "f_key.RemoveScope: Scope '" + scopeName
				+ "' not found.");
		if (!scope) {
			return false;
		}
	...
	_PerformKey : function(jsEvent) {
		...
		if (method == f_key.DefaultAccessKey) {
			f_core
					.Debug(f_key,
							"_PerformKey: Default accessKey comportement !");
		...
		if (method) {
			...
			if (typeof (component) == "string") {
				...
				if (!cmp) {
					f_core.Error(f_key,
							"_PerformKey: Can not find componentClientId '"
									+ component + "'.");
				} else {
					component = cmp;
					def._component = cmp;
			...
			if (component) {
				try {
					var r = method.call(def._component, jsEvent,
							def._parameter, window);
					if (r === true) {
						ret = true;
					}
					...
					f_core.Error(f_key,
							"_PerformKey: Exception/keyHandler, method="
									+ method, ex);
	...
	_SearchDef : function(jsEvent, altKey) {
		...
		if (keyChar) {
			...
			if (keyHandlers) {
				if (currentScope._altKey == jsEvent.altKey) {
					...
					f_core.Debug(f_key, "_SearchDef: KeyHandlers(" + keyChar
							+ "/alt=" + jsEvent.altKey + ")=" + def);
		...
		f_core.Debug(f_key, "_SearchDef: Accelerator(" + keyChar + "/"
				+ keyCode + ")=" + def);
	...
	_SearchAccelerator : function(scope, keyChar, keyCode, jsEvent) {
		...
		if (true) {
			...
			if (false) {
				console.log("altKey=", jsEvent.altKey, " ctrlKey=",
						jsEvent.ctrlKey, " WIN1=", jsEvent
								.getModifierState("Win"), " WIN2=", jsEvent
								.getModifierState("Meta"), " WIN3=", jsEvent
								.getModifierState("OS"));
			}

			if (jsEvent.getModifierState("OS")) {
				mask |= f_key.KF_OS;
			}

			if (jsEvent.getModifierState("Meta")) {
				mask |= f_key.KF_META;
		...
		if (f_core.IsGecko() || f_core.IsWebkit()) {
			try {
				styleSheet.insertRule(
						".f_accessKey { text-decoration: underline }",
	...
	IsModifierKey : function(keyCode) {
		if (keyCode == f_key.VK_SHIFT || keyCode == f_key.VK_CONTROL ||
