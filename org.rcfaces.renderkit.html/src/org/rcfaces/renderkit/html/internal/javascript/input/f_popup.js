...
var __statics = {
	...
	Ie_GetPopup : function(doc, type, readyFunction) {
		f_core.Debug(f_popup, "Ie_GetPopup: Prepare popup for document=" + doc
				+ " type=" + type);
		...
		if (type == f_popup.IE_FRAME_POPUP_TYPE) {
			...
			iframe.onreadystatechange = function() {
				f_core.Debug(f_popup,
						"Ie_GetPopup.readyStateChange: frame created");
		...
		f_core.Assert(popup,
				"f_popup.Ie_GetPopup: Invalid popup from document=" + doc);
	...
	RegisterWindowClick : function(callbacks, component, popup, keyProvider) {
		f_core.Assert(component,
				"f_popup.RegisterWindowClick: Component parameter is null !");
		f_core.Assert(typeof (callbacks) == "object",
				"f_popup.RegisterWindowClick: Callback parameter is null !");

		f_core.Debug(f_popup,
				"RegisterWindowClick: Register callbacks on component='"
						+ component.id + "'.");
		...
		f_core.Debug(f_popup, "RegisterWindowClick: Register popup on "
				+ component.id);
		...
		if (oldComponent) {
			...
			if (oldCallbacks) {
				f_core.Debug(f_popup,
						"RegisterWindowClick: exit old component "
								+ oldComponent.id);
		...
		if (f_popup.Ie_enablePopup()) {
			...
			if (keyProvider) {
				keyProvider.f_insertEventListenerFirst(f_event.KEYDOWN,
						f_popup._OnKeyDown);
				keyProvider.f_insertEventListenerFirst(f_event.KEYUP,
						f_popup._OnKeyUp);
				keyProvider.f_insertEventListenerFirst(f_event.KEYPRESS,
						f_popup._OnKeyPress);
			}
		...
		f_core.AddEventListener(doc, "contextmenu",
				f_popup._Gecko_OnContextMenu, doc);
		return true;
	},
	/**
	 * @method private static
	...
	UnregisterWindowClick : function(component) {
		f_core.Debug(f_popup, "Unregister callbacks on component='"
				+ component.id + "'.");
		...
		if (f_popup.Ie_enablePopup()) {
			if (keyProvider) {
				keyProvider.f_removeEventListener(f_event.KEYDOWN,
						f_popup._OnKeyDown);
				keyProvider.f_removeEventListener(f_event.KEYUP,
						f_popup._OnKeyUp);
				keyProvider.f_removeEventListener(f_event.KEYPRESS,
						f_popup._OnKeyPress);
			}
		...
		f_core.RemoveEventListener(doc, "mousedown",
				f_popup._Gecko_OnMouseDown, doc);
		f_core.RemoveEventListener(doc, "click", f_popup._Gecko_OnClick, doc);
		f_core
				.RemoveEventListener(doc, "dblclick", f_popup._Gecko_OnClick,
						doc);
		...
		f_core.RemoveEventListener(doc, "contextmenu",
				f_popup._Gecko_OnContextMenu, doc);
	...
	_Gecko_OnMouseDown : function(evt) {
		...
		f_core.Debug(f_popup, "_OnMouseDown on " + this + " target=" + target
				+ "/" + target.className + "  popupComponent="
				+ f_popup.Component);
		...
		if (found) {
			...
			evt.cancelBubble = true; // On accepte l'evenement, mais on le
			// passe surtout pas au parent !
			return true;
		}
		...
		// On Poursuit l'evenement ? finalement OUI, car c'est comme ca sur le
		// bureau Windows !
		// evt.cancelBubble=true; // A VERIFIER: On accepte l'evenement, mais on
		// le passe surtout pas au parent !
		return false;
	},
	/**
	...
	IsChildOfDocument : function(target, event) {
		f_core.Debug(f_popup, "IsChildOfDocument: Search parent target='"
				+ target + "' document='" + f_popup.Popup + "'.");
		...
		if (!popupDocument) {
			...
			f_core.Debug(f_popup,
					"IsChildOfDocument: no popup document => false");
			return false;
		}

		for (; target; target = target.parentNode) {
			f_core.Debug(f_popup, "IsChildOfDocument: Test child '" + target
					+ "' #" + target.id + " popupObject='"
					+ target._popupObject + "' popupParent='"
					+ target._popupParent + "'");
			...
			if (target == popupDocument) {
				...
				f_core.Debug(f_popup,
						"IsChildOfDocument: parent is popup document => true");
				return true;
			}
			...
			if (target._menu == popupDocument || target._popupObject) {
				...
				f_core.Debug(f_popup,
						"IsChildOfDocument: menu or popupObject => true");
				return true;
			}

			var isPopupLock = target.f_isPopupLock; // Le composant qui a ouvert
			// la popup peut s'inclure
			// dans la popup
			...
			if (typeof (isPopupLock) == "function") {
				...
				if (cret === false) {
					// Notre composant indique de ne pas considerer la target
					// dans la popup

					f_core.Debug(f_popup,
							"IsChildOfDocument: f_isPopupLock returns false");
					return false;
				}

				if (cret === true) {
					// Notre composant indique que la target est bien dans la
					// popup
					f_core.Debug(f_popup,
							"IsChildOfDocument: f_isPopupLock returns true");
					return true;
				}
			}
		}
	...
	_Ie_OnMouseDown : function(evt) {
		...
		f_core.Debug(f_popup, "_Ie_OnMouseDown: click on " + this
				+ " fromElement=" + evt.srcElement + "/"
		...
		f_core.Debug(f_popup,
				"_Ie_OnMouseDown: click outside: close the popup !");
		var clb = f_popup.Callbacks;
		f_popup.Callbacks = undefined;
	...
	_Gecko_OnClick : function(evt) {
		var target = evt.target || evt.srcElement;
		f_core.Debug(f_popup, "OnClick: click on " + this + " target=" + target
				+ "/" + target.className);
	...
	_Gecko_OnBlur : function(evt) {
		var target = evt.target || evt.srcElement;
		f_core.Debug(f_popup, "OnBlur on " + this + " target=" + target + "/"
				+ target.className);
	...
	_Gecko_OnFocus : function(evt) {
		...
		var target = evt.target || evt.srcElement;
		f_core.Debug(f_popup, "_Gecko_OnFocus: on " + this + " target="
	...
	_OnKeyDownJs : function(evt) {
		...
		var component = f_popup.Component;
		if (!component) {
			f_core.Debug(f_popup, "_OnKeyDownJs: keyCode=" + evt.keyCode
					+ " on " + this + " no component");
		...
		f_core.Debug(f_popup, "_OnKeyDownJs: keyCode=" + evt.keyCode + " on "
				+ this + " component:" + component + " target:" + target);
		...
		if (evt.altKey) { // ?
			try {
				...
				f_core.Error(f_popup,
						"_OnKeyDownJs: Exit callback throws exception", x);
		...
		try {
			...
			f_core.Error(f_popup,
					"_OnKeyDownJs: KeyDown callback throws exception", x);
		}
	...
	_OnKeyUpJs : function(evt) {
		...
		var component = f_popup.Component;
		if (!component) {
			f_core.Debug(f_popup, "_OnKeyUpJs[" + evt.keyCode + "] on " + this
					+ " no component");
		...
		try {
			...
			f_core.Error(f_popup,
					"_OnKeyUpJs: KeyUp callback throws exception", x);
		}
	...
	_OnKeyPressJs : function(evt) {
		...
		var component = f_popup.Component;
		if (!component) {
			f_core.Debug(f_popup, "_OnKeyPressJs[" + evt.keyCode + "] on "
					+ this + " no component");
		...
		f_core.Debug(f_popup, "_OnKeyPressJs[" + evt.keyCode + "] on " + this
				+ " component:" + component + " target:" + target);
		...
		try {
			...
			f_core.Error(f_popup,
					"_OnKeyPressJs: KeyPress callback throws exception", x);
		}
	...
	VerifyLock : function() {
		if (f_popup.Ie_enablePopup()) {
			...
			f_core.Debug(f_popup, "VerifyLock: test popup ! popup=" + popup
					+ " open=" + popup.isOpen);
			...
			if (popup && !popup.isOpen) {
				...
				f_core.Debug(f_popup,
						"VerifyLock: close not opened popup ! (tagName="
								+ popup.tagName + ")");
				...
				return false; // FALSE= La popup a �t� ferm�e, on revient en
				// scope normal
			}
		}
	...
	Ie_openPopup : function(popup, positionInfos) {
		f_core.Assert(typeof (popup) == "object",
				"f_popup.Ie_openPopup: Invalid popup parameter '" + popup
						+ "'.");
		f_core.Assert(typeof (positionInfos) == "object",
				"f_popup.Ie_openPopup: Invalid positionInfos parameter '"
						+ positionInfos + "'.");
		f_core.Assert(typeof (positionInfos.position) == "number",
				"f_popup.Ie_openPopup: Invalid positionInfos.position parameter '"
						+ positionInfos.position + "'.");

		f_core.Debug(f_popup, "Ie_openPopup: open popup '" + popup
				+ "' positionInfos=" + positionInfos.position);
		...
		// f_core.Debug(f_popup, "Ie_openPopup: Popup position x="+popupX+"
		// y="+popupY+" w="+popupW+" h="+popupH+" component="+popupComponent);
		...
		var scrollPosition = f_core
				.GetScrollOffsets(popupComponent.ownerDocument);
		bw += scrollPosition.x;
		bh += scrollPosition.y;
		...
		f_core.Debug(f_popup, "Ie_openPopup: Open popup x=" + popupX + " y="
				+ popupY + " w=" + popupW + " h=" + popupH
				+ " componentPosition=" + popupComponent.id + "/"
				+ popupComponent.tagName);
	...
	_Ie_unload : function(evt) {
		...
		f_core.Debug(f_popup, "_Ie_unload: Unload popup '" + this.id
				+ "' rootPopup=" + doc._rootPopup);
	...
	Ie_releasePopup : function(popup) {
		if (popup.tagName == "IFRAME") {
			// On le traite en asynchrone pour des problemes d'evenement en
			// cours de traitement
			window.setTimeout(function() {
	...
	Gecko_openPopup : function(popup, positionInfos) {
		f_core.Assert(typeof (popup) == "object",
				"f_popup.Gecko_openPopup: Invalid popup parameter '" + popup
						+ "'.");
		f_core.Assert(typeof (positionInfos) == "object",
				"f_popup.Gecko_openPopup: Invalid positionInfos parameter '"
						+ positionInfos + "'.");
		f_core.Assert(typeof (positionInfos.position) == "number",
				"f_popup.Gecko_openPopup: Invalid positionInfos.position parameter '"
						+ positionInfos.position + "'.");
		...
		if (positionInfos.maxPopupHeight
				&& popup.offsetHeight > positionInfos.maxPopupHeight) {
			heightSetted = true;
			offsetHeight = positionInfos.maxPopupHeight;
		}

		popup.style.display = "block"; // On remet le display block pour
		// pouvoir avoir la taille !
		...
		switch (positionInfos.position) {
			...
			case f_popup.MOUSE_POSITION :
				...
				f_core.Debug(f_popup, "Gecko_openPopup: (mouse position) X="
						...
						+ " eventY=" + eventPos.y + " cursorPosX="
						+ cursorPos.x + " cursorPosY=" + cursorPos.y);
		...
		f_core.Debug(f_core, "Gecko_openPopup: bw=" + bw + " bh=" + bh
				...
				+ " popupWidth=" + offsetWidth + " popupHeight="
				+ popup.offsetHeight);
