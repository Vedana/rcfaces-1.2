	...
	private static void writeInitIds(IJavaScriptWriter jsWriter,
		...
		for (Iterator it = initializeIds.iterator(); it.hasNext();) {
			...
			if ((componentId.mode & JavaScriptEnableModeImpl.ONINIT) == 0) {
				...
				// Si pas de messages
				// ou s'il y a des messages et que le composant ne traite pas les messages ...
				if (hasMessages == false || (componentId.mode & JavaScriptEnableModeImpl.ONMESSAGE) == 0) {
					
					// On met dans others tous les autres composants qui doivent etre initialisés hors ONINIT
					if (others == null) {
