...
//	private String newVar;
	 ...
	 */
	protected String getCssClassBase(MessageDialogComponent component, FacesContext facesContext) {
	...
	protected SelectItemsContext createJavaScriptContext() throws WriterException {
        ...
        IComponentRenderContext componentRenderContext = javaScriptWriter.getHtmlComponentRenderContext();

        return new SelectItemsJsContext(this, componentRenderContext, getComponent(), null);
	}
	...
	protected void encodeComponentsBegin() throws WriterException {
		...
		if (styleClass != null) {
			...
			javaScriptWriter.writeMethodCall("f_setCssClassBase").writeString(styleClass).writeln(");");			
		}
		styleClass = component.getStyleClass(facesContext);
		if (styleClass != null) {
			// titi.f_setStyleClass(styleClass);
			javaScriptWriter.writeMethodCall("f_setStyleClass").writeString(styleClass).writeln(");");			
		}
		
		int priority = component.getPriority(facesContext);
		if (priority != 0) {
			javaScriptWriter.writeMethodCall("f_setPriority").writeInt(priority).writeln(");");			
		}
		...
		if (contentAccessors instanceof IImageAccessors) {
			...
			if (imageAccessor != null) {
				String imageSrc = imageAccessor.resolveURL(facesContext, null, null);
				if (imageSrc != null) {
					javaScriptWriter.writeMethodCall("f_setImageURL").writeString(imageSrc).writeln(");");			
				}
			}
		}
	...
	protected void encodeComponentsEnd() throws WriterException {
		...
		// titi.open(callback)
        String jsCallBack = component.getJsCallback(facesContext);
        if (jsCallBack == null) {
        	jsCallBack = "";
        }
		javaScriptWriter.writeMethodCall("f_open").write(jsCallBack).writeln(");");
	...
	public int encodeNodeBegin(UIComponent component, SelectItem selectItem,
        ...
        javaScriptWriter.writeMethodCall("f_addAction").writeString(value).write(",").writeString(text).write(",").writeBoolean(selectItem.isDisabled()).write(",").writeBoolean(value == defaultValue).writeln(");");
