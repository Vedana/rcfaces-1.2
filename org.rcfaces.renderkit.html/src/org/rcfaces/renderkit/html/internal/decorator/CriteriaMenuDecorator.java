...
public class CriteriaMenuDecorator extends SubMenuDecorator {
	...
	public CriteriaMenuDecorator(UIComponent component, String menuId,
		...
		super(component, menuId, suffixMenuId, removeAllWhenShown, itemImageWidth,
				itemImageHeight);
	}
	...
	protected SelectItemsContext createJavaScriptContext()
		...
		if(parentComponent instanceof ICriteriaContainer ){
			...
			Object selectedCriteriaValues = 
			CollectionTools.valuesToSet(
					container.getCriteriaConfiguration().getSelectedValues(), true);
			if (selectedCriteriaValues != null) {
				((MenuComponent) component).setCheckedValues(selectedCriteriaValues);
			}
		}
	...
	protected Iterator iterateNodes(UIComponent component)
		...
		if(parentComponent instanceof ICriteriaContainer ){
			ICriteriaContainer container = (ICriteriaContainer) parentComponent;
			ISelectedCriteria[] selectedCritera =  CriteriaTools.listSelectedCriteria(container.getCriteriaManager());
			CriteriaItem[] criteriaItem =  CriteriaTools.listAvailableCriteriaItems(container.getCriteriaConfiguration(), selectedCritera);
			
			int criteriaCardinality =  container.getCriteriaConfiguration().getCriteriaCardinality(); 
